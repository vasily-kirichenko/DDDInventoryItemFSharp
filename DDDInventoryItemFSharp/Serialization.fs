...
module private JsonNet = 
    ...
    type GuidConverter() = 
        ...
        override x.WriteJson(writer, value, serializer) = 
            ...
            if value <> Guid.Empty
            then writer.WriteValue(value.ToString("N"))
    ...
    type ListConverter() = 
        inherit JsonConverter()
        override x.CanConvert(t: Type) = t.IsGenericType && t.GetGenericTypeDefinition() = typedefof<list<_>>
        override x.WriteJson(writer, value, serializer) = 
        ...
        override x.ReadJson(reader, t, _, serializer) = 
            ...
            let rec make = 
                ...
                | head :: tail -> 
                    FSharpValue.MakeUnion(cases.[1], [|head
                                                       (make tail)|])
            make(collection |> Seq.toList)
    
    type 'a NonEmptyListConverter() = 
        inherit JsonConverter()
        override x.CanConvert(t: Type) = t.IsGenericType && t.GetGenericTypeDefinition() = typedefof<NonEmptyList<_>>
        override x.WriteJson(writer, value, serializer) = 
            let list = value :?> System.Collections.IEnumerable |> Seq.cast
            serializer.Serialize(writer, list)
        override x.ReadJson(reader, t, _, serializer) = 
            let itemType = t.GetGenericArguments().[0]
            let collectionType = typedefof<IEnumerable<_>>.MakeGenericType(itemType)
            let collection = serializer.Deserialize(reader, collectionType) :?> System.Collections.IEnumerable |> Seq.cast<'a>
    ...
    type TupleArrayConverter() = 
        inherit JsonConverter()
        override x.CanConvert(t: Type) = FSharpType.IsTuple(t)
        override x.WriteJson(writer, value, serializer) = 
    ...
    type UnionCaseNameConverter() = 
        inherit JsonConverter()
        override x.CanConvert(t) = FSharpType.IsUnion(t) || (t.DeclaringType <> null && FSharpType.IsUnion(t.DeclaringType))
        override x.WriteJson(writer, value, serializer) = 
            let t = value.GetType()
        ...
        override x.ReadJson(reader, t, _, serializer) = 
            ...
            let readProp(n: string) = 
                read JsonToken.PropertyName |> Option.map(fun v -> 
                                                       if (v :?> string) <> n
                                                       then fail())
    ...
    s.Converters.Add(new NonEmptyListConverter<string>())
    s.Converters.Add(new UnionCaseNameConverter())
    
    let eventType o = 
        let t = o.GetType()
        if FSharpType.IsUnion(t) || (t.DeclaringType <> null && FSharpType.IsUnion(t.DeclaringType))
        then 
            let cases = FSharpType.GetUnionCases(t)
